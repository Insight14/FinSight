...
function Analysis({ onNavigate, transactions }) {
  ...
  const normalizeCategory = (tx) => {
    ...
    if (mainCategories.some(cat => cat.label === category)) return category

    const text = `${category} ${String(tx?.description || '')}`.toLowerCase()
  ...
  // small parser for amount strings like '-$98.32' or '+$2400.00'
  const parseAmount = (str) => {
    const n = parseFloat(String(str).replace(/[^0-9.-]/g, ''))
    return isNaN(n) ? 0 : n
  }

  // Calculate actual spending from transactions (use provided transactions or fallback mock)
  const txSource = transactions && transactions.length ? transactions : mockTransactions
  const categorySpending = useMemo(() => {
    const spending = {}
    mainCategories.forEach(cat => {
      spending[cat.label] = txSource
        .filter(tx => normalizeCategory(tx) === cat.label)
  ...
  const piePercents = pieData.map(cat => (pieTotal > 0 ? (cat.value / pieTotal * 100).toFixed(2) : '0.00'))

  const formatMoney = (n) => Number(n || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })

  const resultItems = [
    { label: 'Total Savings', value: `$${formatMoney(available)}` },
    { label: pieData[0]?.label, value: `$${formatMoney(pieData[0]?.value)}` },
    { label: pieData[1]?.label, value: `$${formatMoney(pieData[1]?.value)}` },
    { label: pieData[2]?.label, value: `$${formatMoney(pieData[2]?.value)}` },
  ]
      ...
      <nav className="analysis-topbar">
        <div className="topbar-content">
          ...
          <div className="topbar-right">
            ...
            <button className="icon-btn" title="View Transactions" onClick={() => onNavigate && onNavigate('transactions')}>📄</button>
          ...
          <div className="section-card">
            <h3 className="card-title">Budget Tracker</h3>
              ...
              <div className="stat-row">
                ...
                <span className="stat-value">${formatMoney(totalBudget)}</span>
              </div>
              <div className="stat-row">
                <span className="stat-label">Total Spent</span>
                <span className="stat-value spent">${formatMoney(totalSpent)}</span>
              </div>
              <div className="stat-row">
